...
TEST(GroupTest, CompilePairs) {
    ...
    EXPECT_THAT(make_stream::range(0, 6) | pairwise() | to_vector(),
                ElementsAre(G{0, 1}, G{2, 3}, G{4, 5}));
    EXPECT_THAT(make_stream::range(0, 6) | group<2>() | to_vector(),
                ElementsAre(G{0, 1}, G{2, 3}, G{4, 5}));
    EXPECT_THAT(make_stream::range(0, 5) | pairwise() | to_vector(),
                ElementsAre(G{0, 1}, G{2, 3}));
}

TEST(GroupTest, CompileTriples) {
    using G = std::tuple<int, int, int>;
    EXPECT_THAT(make_stream::range(0, 6) | group<3>() | to_vector(),
                ElementsAre(G{0, 1, 2}, G{3, 4, 5}));
    EXPECT_THAT(make_stream::range(0, 7) | group<3>() | to_vector(),
                ElementsAre(G{0, 1, 2}, G{3, 4, 5}));
    EXPECT_THAT(make_stream::range(0, 8) | group<3>() | to_vector(),
                ElementsAre(G{0, 1, 2}, G{3, 4, 5}));
}
...
TEST(GroupTest, CompileFourTuples) {
    ...
    EXPECT_THAT(make_stream::range(0, 8) | group<4>() | to_vector(),
